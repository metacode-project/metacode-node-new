...
后端默认端口 `2023`，tRPC 入口为 `http://localhost:2023/rpc`，OpenAPI 文档为 `http://localhost:2023/doc`。前端访问入口为 `http://localhost:5174"`。
...
- tRPC 入口: `http://localhost:2023/rpc`
- OpenAPI 文档: `http://localhost:2023/doc`
...
**测试**
Playwright 默认使用 `PLAYWRIGHT_TEST_BASE_URL`，未设置时为 `http://localhost:5174`。
如需修改测试地址，设置环境变量：

```bash
PLAYWRIGHT_TEST_BASE_URL="http://localhost:5174" pnpm test:e2e
```
